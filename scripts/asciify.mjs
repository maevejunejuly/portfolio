...
const CHAR_ASPECT = 0.95;

// Opening of Nannaya's Adi Parvam, Andhra Mahabharatamu.
const TEXT = [
  ...
  "జోజయశాలిశౌర్యుడు విశుద్ధయశశ్శరదిందుచంద్రికా",
...
let sp = 0;
const GLYPHS = [
  ...new Intl.Segmenter("te", { granularity: "grapheme" }).segment(TEXT),
]
  .map((g) => g.segment)
  .filter((g) => !/\s/.test(g) || ++sp % 2 === 0);
...
for (let y = 0; y < img.h; y++) {
  ...
  for (let x = 0; x < img.w; x++) {
    ...
    const g = img.rgb[i + 1];
    const b = img.rgb[i + 2];
    const ch = GLYPHS[(y * img.w + x) % GLYPHS.length];
    const ci = quantise(r, g, b);
    if (cur && cur[0] === ci) cur[1].push(ch);
    else {
      cur = [ci, [ch]];
      runs.push(cur);
    }
...
const body = rows
  .map((runs) => `  [${runs.map(([c, g]) => `["${DIGITS[c]}",${JSON.stringify(g)}]`).join(",")}],`)
  .join("\n");
