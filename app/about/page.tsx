...
export default function About() {
  return (
    <div className="mx-auto max-w-shell overflow-hidden bg-bg text-fg">
      <div className="grid grid-cols-1 gap-12 px-4 pt-10 sm:px-7 lg:grid-cols-[300px_minmax(0,1fr)_244px] lg:gap-16">
        <div className="flex max-w-[320px] flex-col gap-8">
          <p className="text-meta leading-[1.7] opacity-70 [text-wrap:pretty]">
            ...
            student studying Electrical Engineering &amp; Computer Science. She
            is searching for new grad roles in full-stack development, with a
            specialty in natural language processing, AI application, and UI/UX.
          </p>
        ...
        <div className="flex min-w-0 flex-col">
          {experience.map((item) => (
            <div
              key={item.role}
              className="mx-[-10px] flex items-baseline justify-between gap-6 border-t border-line px-2.5 py-3.5 first:border-t-0 hover:bg-inv-bg hover:text-inv-fg"
            >
              <span className="truncate text-meta">{item.role}</span>
              <span className="whitespace-nowrap text-label opacity-60">
                {item.when}
              </span>
            </div>
          ))}
      ...
      <div className="px-4 pt-32 sm:px-7">
        <div className="flex max-w-[320px] flex-col gap-8">
          <p className="text-meta leading-[1.7] opacity-70 [text-wrap:pretty]">
      ...
      <div className="mt-40">
        <div
          className="overflow-hidden whitespace-nowrap px-4 font-display text-[clamp(48px,11vw,105px)] uppercase leading-[0.86] tracking-wordmark sm:px-7"
          style={{ color: "rgba(20,19,15,.28)" }}
        >
          Maevejunejuly
        </div>
        <Footer />
